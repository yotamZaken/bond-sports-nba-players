...
export default function PlayerItem({ player, isFavoritesList }: PlayerItemProps) {
    ...
    return (
        <div className={clsx("grid grid-cols-2 gap-3 p-3 max-h-14 justify-items-center content-start rounded", "border border-1 border-orange-500")  }>
            <p>{capitalize(player.first_name)} {capitalize(player.last_name)}</p>
            <button className="h-5 w-5" onClick={handleFavClick}>
